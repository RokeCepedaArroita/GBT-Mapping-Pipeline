...
def SetupSimulation():
    ...
    print('Remember, we have two beams, so double the number of days to simulate this effect!',flush=True)
    print('We may have to request the full bandwidth of this mode!')

    # Generate telescope feed positions
    frequencies = [13.7] # GHz
    tsys = 30 # K
    sample_rate = 25 # Hz
    band_width  = 3.4e9 # Hz
    ...
    # Observing
    daisy_radius = 0.4
    daisy_infinity_time = 60.0 # in seconds - cannot be faster than 15 in any case!!!
    observation_length_sec = daisy_infinity_time*22*20 # in seconds
    print(f'Time for a single observation = {observation_length_sec/3600.:.2f} hours',flush=True)
    nDays = 8
    nRepointings = 1 # x2 since there are two beams, and x3 to triple the time requested

    # Image and daisy center
    crval = [192.41, -11.51]

    # Pixelization
    nyquist = 1 # pixels per FWHM beamwidth
    ...
    if frequencies[0] == 4.85:
        scale_factor = 10 # this is where the degradation factors come
    elif frequencies[0] == 13.7:
        scale_factor = 15 # this is where the degradation factors come
    ...
    nosc = 22*20*2 # THE TOTAL NUMBER OF TURNS IS TWICE THAT IN EACH OBSERVING TURN
    n_scans = 2
    import math
...
if __name__ =="__main__":
    ...
    for i in range(nRepeats):
        ...
        if mapper.__name__ == 'NaiveMapper':
            ...
            # Write fits
            hdul.writeto(f'gbt_outputs/daisy_repeats{i:03d}_{telescope.frequencies[0]}GHz_beampix.fits',overwrite=True)
            print(f'Saved gbt_outputs/daisy_repeats{i:03d}_{telescope.frequencies[0]}GHz_beampix.fits!')
