...
def plot_map_thesis(image, nhits, rms, wcs, zoom, title, contourdata=None, dark_clouds=None, central_coords=None, frequency=None):
    ...
    # Plot hits
    plt.figure(5, figsize=(figsize[0],figsize[1]))
    ax = plt.subplot(projection=wcs)
    ax.set_facecolor(('#a9a9a9'))
    plt.imshow(nhits, vmin=np.nanmin(nhits), vmax=15, origin='lower')
    ...
    plt.ylabel('Galactic Latitude')
    plt.title(f'{band}-Band Exposure [' + r'$\mathrm{sec}$' + ']')
    ...
    # Save figure
    plt.savefig(f'./simresults/{title}_explin.png', bbox_inches='tight', pad_inches=0, dpi=600)
    plt.savefig(f'./simresults/{title}_explin.pdf', bbox_inches='tight', pad_inches=0, dpi=600)
